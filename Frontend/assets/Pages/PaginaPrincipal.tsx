...
const API_URL =
  'https://backend-or-main-production-2a36.up.railway.app';
...
const getStyles = (colors: typeof coresClaro) =>
  StyleSheet.create({
    safe: {
      flex: 1,
...
const PaginaPrincipal = ({ navigation }: any) => {
  ...
  const [expandidoId, setExpandidoId] =
   ...
   */
  const dataSelecionada = criarData(
    anoSel,
    mesSel,
    diaSel
  );
  ...
  /*
   * SOMENTE os remédios do dia selecionado.
   *
   * Essa é a principal correção do problema.
   */
  const remediosDoDia = remedios.filter(
    r => r.data === dataSelecionada
  );

  const tomados = remediosDoDia.filter(
    r => r.tomado
  ).length;
  ...
  function mudarMes(delta: number) {
    ...
    /*
     * Mantém o dia válido dentro do novo mês.
     */
    const diasNoNovoMes = getDiasNoMes(
      novoAno,
      novoMes
    );

    if (diaSel > diasNoNovoMes) {
      setDiaSel(diasNoNovoMes);
    }
  }
  ...
  async function salvarRemedio() {
    ...
    try {
      /*
       * Envia a data selecionada para a API também.
       */
      ...
      /*
       * Mesmo que a API não devolva a data,
       * nós garantimos que o remédio salvo
       * localmente tenha a data correta.
       */
      adicionarRemedio({
        ...json.data,
        data: dataSelecionada,
      ...
      >
        ...
        <View style={styles.progressCard}>
          ...
          <Text style={styles.progressLabel}>
            {total === 0
              ? t(
                  'principal.nenhumAdicionado'
                )
              : tomados === total
              ? t(
                  'principal.todosTomados'
                )
              : `${
                  total - tomados
                } ${
        ...
        <View style={styles.card}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>
              {t('principal.remedios')} —{' '}
              {String(diaSel).padStart(
                2,
                '0'
              )}
              /
            ...
            return (
              <View key={r.id}>
                <View style={styles.remedioRow}>
                  ...
                  <TouchableOpacity
                    style={
                      styles.remedioNomeTouch
                    }
                    onPress={() =>
                      toggleObservacao(r.id)
                    }
                    disabled={
                      !r.observacoes
                    }
        ...
        >
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>
              {t(
                'principal.novoRemedio'
              )}
            </Text>

            <Text style={styles.inputLabel}>
              {t(
                'principal.nomeRemedio'
              )}
            </Text>
            ...
            <TextInput
              ...
              placeholder={t(
                'principal.nomeRemedioPlaceholder'
              )}
