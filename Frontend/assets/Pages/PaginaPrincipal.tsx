...
const getStyles = (colors: typeof coresClaro) =>
  StyleSheet.create({
    ...
    inputHint: { fontSize: 11, color: colors.textSecondary, marginBottom: 10, marginLeft: 4 },
...
const PaginaPrincipal = ({ navigation }: any) => {
  ...
  function mudarMes(delta: number) {
    ...
    setMesSel(novoMes);
    const diasNoNovoMes = getDiasNoMes(novoAno, novoMes);
    if (diaSel > diasNoNovoMes) setDiaSel(diasNoNovoMes);
  }
  ...
  async function salvarRemedio() {
    ...
    if (!novoNome.trim()) { setErroNome(t('principal.informeNome')); valido = false; }
    else { setErroNome(''); }
    ...
    try {
      const response = await fetch(`${API_URL}/remedio`, {
        ...
        body: JSON.stringify({
          ...
          ...(frequenciaNum && { frequenciaHoras: frequenciaNum }),
        }),
      });
        ...
        <View style={styles.card}>
          ...
          <View style={styles.calGrid}>
            {celulas.map((dia, i) => {
              ...
              return (
                <TouchableOpacity key={i} style={[styles.calCell, isSel && styles.calCellSelected, isHoje && !isSel && styles.calCellToday]} onPress={() => dia && selecionarDia(dia)} disabled={!dia}>
      ...
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.inputLabel}>Frequência (opcional)</Text>
            <TextInput
              style={styles.input}
              placeholder="Ex: 8 (a cada 8 horas)"
              placeholderTextColor={colors.textSecondary}
              value={novaFrequencia}
              onChangeText={txt => setNovaFrequencia(txt.replace(/\D/g, ''))}
              keyboardType="numeric"
              maxLength={2}
            />
